...
/**
 * 
 * SysTableAccessInfoService
 *
 * @Description: SysTableAccessInfoService
 * @author graysonzhang
 * @data 2019-08-08 14:24:32
 *
 */
@SuppressWarnings("rawtypes")
@Service
public class SysTableAccessInfoService extends DBBaseOperation implements StorageService {
	
	@Autowired
	private SysTableAccessInfoMapper mapper;
	...
	public void createSchema() {
	    ...
	    mapper.createTable(DBStaticTableConstants.SYS_TABLE_ACCESS_TABLE);
		String detailTableName = DBStaticTableConstants.SYS_TABLE_ACCESS_TABLE + DBStaticTableConstants.SYS_DETAIL_TABLE_POST_FIX;
    ...
    public void batchSave(String tableName, List list) {
        ...
        for (int index = 0; index < list.size();) {
            ...
            if (systemPropertyConfig.getBatchCount() >= list.size() - index) {
                ...
                mapper.batchInsert((List<SysTableAccessInfo>)list.subList(index, batchLastIndex));
                break;
            } else {
                mapper.batchInsert((List<SysTableAccessInfo>)list.subList(index, batchLastIndex));
    ...
    public void batchSaveDetail(String tableName, List list) {
        ...
        for (int index = 0; index < list.size();) {
            ...
            if (systemPropertyConfig.getBatchCount() >= list.size() - index) {
                batchLastIndex = list.size();
                mapper.batchInsertDetail((List<SysTableAccessInfo>)list.subList(index, batchLastIndex));
                break;
            } else {
                mapper.batchInsertDetail((List<SysTableAccessInfo>)list.subList(index, batchLastIndex));
                index = batchLastIndex;
