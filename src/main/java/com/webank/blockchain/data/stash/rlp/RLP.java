 ...
 * <p>
 * The purpose of RLP is to encode arbitrarily nested arrays of binary data, and RLP is the main encoding method used to
 * serialize objects in Ethereum. The only purpose of RLP is to encode structure; encoding specific atomic data types
 ...
 * <p>
 * For example, an empty string is an item, as is the string containing the word "cat", a list containing any number of
 * strings, as well as more complex data structures like ["cat",["puppy","cow"],"horse",[[]],"pig",[""],"sheep"]. Note
 * that in the context of the rest of this article, "string" will be used as a synonym for "a certain number of bytes of
 * binary data"; no special encodings are used and no knowledge about the content of the strings is implied.
 * <p>
...
public class RLP {
    ...
    protected static final byte[] EMPTY_ELEMENT_RLP = encodeElement(new byte[0]);
    ...
    /**
     * Parse length of long item or list. RLP supports lengths with up to 8 bytes long, but due to java limitation it
     * returns either encoded length or {@link Integer#MAX_VALUE} in case if encoded length is greater
     *
     ...
     */
    public static int calcLength(int lengthOfLength, byte[] msgData, int pos) {
        ...
        // Console.log("length is {}, lengthOfLength {}, msgData length {}, pos {}.", length, lengthOfLength,
        // msgData.length, pos);
        return length;
    }
    ...
    /**
     * Decodes RLP with list without going deep after 1st level list (actually, 2nd as 1st level is wrap only)
     *
     * So assuming you've packed several byte[] with {@link #encodeList(byte[]...)}, you could use this method to unpack
     * them, getting RLPList with RLPItem's holding byte[] inside
     * 
    ...
    public static int calOffsetLength(byte[] msgData, int pos) {
    ...
    private static void verifyLength(int suppliedLength, int availableLength) {
        if (suppliedLength > availableLength) {
            throw new RuntimeException(String.format(
                    "Length parsed from RLP (%s bytes) is greater " + "than possible size of data (%s bytes)",
                    suppliedLength, availableLength));
        }
    }
    ...
    public static DecodeResult decode(byte[] data, int pos) {
        ...
        if (prefix == OFFSET_SHORT_ITEM) { // 0x80
            ...
            int lenbytes = ByteUtil.byteArrayToInt(copyOfRange(data, pos + 1, pos + 1 + lenlen)); // length of encoded
                                                                                                  // bytes
            // check that length is in payload bounds
            ...
            int lenlist = ByteUtil.byteArrayToInt(copyOfRange(data, pos + 1, pos + 1 + lenlen)); // length of encoded
                                                                                                 // bytes
    ...
    public static LList decodeLazyList(byte[] data) {
        LList l = decodeLazyList(data, 0, data.length);
        if(l == null) {
            return null;
        }
        else {
    ...
    public static LList decodeLazyList(byte[] data, int pos, int length) {
        ...
        while (pos < end) {
            ...
            if (prefix == OFFSET_SHORT_ITEM) { // 0x80
                ...
                int lenlen = prefix - OFFSET_LONG_ITEM; // length of length the encoded bytes
                int lenbytes = ByteUtil.byteArrayToInt(copyOfRange(data, pos + 1, pos + 1 + lenlen)); // length of
                                                                                                      // encoded bytes
                // check that length is in payload bounds
                verifyLength(lenbytes, data.length - pos - 1 - lenlen);
                ...
                int lenlen = prefix - OFFSET_LONG_LIST; // length of length the encoded list
                int lenlist = ByteUtil.byteArrayToInt(copyOfRange(data, pos + 1, pos + 1 + lenlen)); // length of
                                                                                                     // encoded bytes
                // check that length is in payload bounds
    ...
    public static byte[] encodeElement(byte[] srcData) {
        ...
        if (ArrayUtils.isEmpty(srcData)) {
            ...
            // [0x00]
        } else if (srcData.length == 1 && srcData[0] == 0) {
            return srcData;
